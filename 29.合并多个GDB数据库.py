...
gdbpath=outpath+"\\"+outgdbname+".gdb"
if arcpy.Exists(gdbpath): #判断是否已存在将新建的GDB数据库
    pass
else:
...
for gdb in gdblist: #循环待合并GDB数据库
    ...
    for ds in datasets: #循环要素数据集
        ...
        for fc in fcs:
            if arcpy.Exists(gdbpath+"\\"+ds + "\\" + fc):
                pass
            else:
                arcpy.FeatureClassToFeatureClass_conversion(fc,gdbpath+"\\"+ds,fc)
                print("        " + fc)
                arcpy.DeleteFeatures_management(gdbpath+"\\"+ds + "\\" + fc)
...
for ds in datasets:
    ...
    for fc in fcs:
        for gdb in gdblist:
            if arcpy.Exists(gdb+"\\"+ds+"\\"+fc):
                arcpy.Append_management(gdb+"\\"+ds+"\\"+fc, gdbpath + "\\" + ds + "\\" + fc,"NO_TEST")
                print('        '+fc+'  '+'合并成功')
